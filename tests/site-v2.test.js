...
const publicChannels = buildPublicChannels(publicJobs, "2026-07-27");
...
execFileSync(process.execPath, ["scripts/build-site.js"], {
  cwd: ROOT,
  env: { ...process.env, POOL_AS_OF_DATE: "2026-07-27" },
  stdio: "pipe",
});
...
const todayIds = new Set(
  builtIssues.filter((issue) => issue.date === "2026-07-27").flatMap((issue) => issue.job_ids)
);
...
execFileSync(process.execPath, ["scripts/build-site.js"], {
  cwd: ROOT,
  env: { ...process.env, POOL_AS_OF_DATE: "2026-07-28" },
  stdio: "pipe",
});
...
assert.match(nextDaySection, /最新更新 <small>2026-07-27<\/small>/);
assert.deepEqual(
